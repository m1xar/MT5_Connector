...
#
#   bash deploy/linux/clone-pool.sh 16
#
...
# two per core they just split the same CPU. Clones carry no caches, logs or
# tester state from the master.
#
...
if [ -n "$(find "$MT5_DIR/master" -iname accounts.dat -print -quit)" ]; then
...
for i in $(seq 1 "$COUNT"); do
  ...
  rm -rf "$MT5_DIR/t$i/Bases"/* "$MT5_DIR/t$i/logs" "$MT5_DIR/t$i/Tester" "$MT5_DIR/t$i/MQL5/Logs" "$MT5_DIR/t$i/MQL5/Files/Temp"
  echo "  t$i created"
done
...
BS='\'
REL="$(printf %s "${MT5_DIR#"$PREFIX/drive_c/"}" | tr / '\\')"
WIN_DIR="C:${BS}${REL}"
PATHS=""
for i in $(seq 1 "$COUNT"); do PATHS="${PATHS}${WIN_DIR}${BS}t$i${BS}terminal64.exe;"; done
